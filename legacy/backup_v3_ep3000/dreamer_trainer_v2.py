...
class DreamerV3AgentV2:
    ...
    def __init__(self, input_dim=145, seq_len=48, embedding_dim=128,
                 ...
                 wm_lr=2e-4, ac_lr=3e-4, jepa_lr=1e-4,
        ...
        self.jepa = TSJEPA(
            ...
            embedding_dim=embedding_dim, momentum=0.999  # FIX: 0.99 → 0.999
        ).to(self.device_wm)
    ...
    def train_actor_critic(self, batch):
        ...
        if self.entropy_history:
            ...
            if recent_entropy < self.min_entropy:
                ...
                self.actor_critic.entropy_coeff = min(0.2, self.base_entropy_coeff * 3.0)
            else:
                self.actor_critic.entropy_coeff = max(0.02, self.base_entropy_coeff)
    ...
    def get_action(self, obs, action_mask=None, temperature=1.0, deterministic=False):
        ...
        with torch.no_grad():
            ...
            if deterministic or temperature == 0:
                ...
                tempered_logits = torch.log(probs + 1e-8) / max(temperature, 0.7)  # FIX: floor temp at 0.7
