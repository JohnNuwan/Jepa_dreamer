...
class MultiSymbolEnvV3:
    ...
    def reset(self):
        ...
        self._prev_equity = self.balance  # V5: unrealized PnL
    ...
    def step(self, action):
        ...
        # V5: Unrealized PnL reward - feedback while holding
        if self.positions:
            cur_eq = self.balance + sum(p.unrealized_pnl(current_price) for p in self.positions)
            eq_chg = (cur_eq - self._prev_equity) / FTMO_CONFIG["account_size"]
            reward += eq_chg * 3.0  # 3x running PnL
            self._prev_equity = cur_eq
        else:
            self._prev_equity = self.balance
    ...
    def _open_position(self, direction, entry_price, pos_type, 
        ...
        return 0.25  # V4: incentiver, boosted V3
